...
class Buffer {
    ...
    public Response Process(Request request) {
        // write your code here
    	 ...
    	 if(finish_time_.isEmpty())//when new packet arrives, computer will start processing if empty
    	 {
             finish_time_.add(request.arrival_time + request.process_time);
             return new Response(false, request.arrival_time);
         }
         else {//otherwise if not empty and finish time <= arrival time 
             while(!finish_time_.isEmpty() && finish_time_.get(0)<= request.arrival_time){
                 finish_time_.remove(0);  
             }

             if(finish_time_.size() == size_)
                 return new Response(true, -1);//confirm drop
             else if(finish_time_.isEmpty())//otherwise process
                 finish_time_.add(request.arrival_time + request.process_time);
             else//
